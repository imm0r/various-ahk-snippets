...
overlay.AttachToWindow("ahk_id " GetHwnd( "LDPlayer", "dnplayer.exe" ), 2)
if (!WinExist("ahk_id " GetHwnd( "LDPlayer", "dnplayer.exe" ))) {
	msgbox % "Please open a notepad window and press OK to reload"
	reload
}
WinActivate, % "ahk_id " GetHwnd( "LDPlayer", "dnplayer.exe" )

overlay := new ShinsOverlayClass("ahk_id " GetHwnd( "LDPlayer", "dnplayer.exe" ))
