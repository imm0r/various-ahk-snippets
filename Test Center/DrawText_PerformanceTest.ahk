...
loop, 5
{
	...
	while (y < sy + sh) {
		if (oL.BeginDraw()) {
				oL.DrawText("notlined text - " f := A_Index, x, y, 24, 0xFFFFFFFF, "Arial", "olFFFF0000")
				oL.EndDraw()
		}
		y++
	}
...
conc := "using no outlines is " round((fps2 / fps1) * 100) " times faster"

; final result
if (oL.BeginDraw()) {
   oL.FillRoundedRectangle(floor(sw * 0.25), floor(sh * 0.7) -10,695,210,5,2,0x66151A24)
   oL.DrawText("DrawText performance benchmark:", floor(sw * 0.25), floor(sh * 0.7), 30, 0xFF00FF00, "Arial", "w695 aCenter olFF0000FF")
   oL.DrawText("both ran for "(sy + sh) * 5 " times!", floor(sw * 0.25), floor(sh * 0.7) + 50, 24, 0xFFFFFFFF, "Arial", "w695 aCenter olFF363636")
   oL.DrawText(bench2 "`n" bench1, floor(sw * 0.26), floor(sh * 0.7) + 100, 24, 0xFFFFFF00, "Tahoma", "w650 aRight olFF000000")
   oL.DrawText(conc, floor(sw * 0.25), floor(sh * 0.7) + 170, 24, 0xFFFFFFFF, "Arial", "w695 aCenter olFF363636")
   oL.EndDraw()
}
return
