...
text := GetLastMsg(fullText)

If InStr(text, "Anti-bot") || InStr(text, "verify with this code")
{
   Loop, % oText.MaxIndex()
   {
...
getClientRect(hWnd, ByRef tX:="", ByRef tY:="", ByRef tW:="", ByRef tH:="")
{
...
ocr(file, lang := "FirstFromAvailableLanguages")
{
   ...
   if (OcrEngineStatics = "") {
      CreateClass("Windows.Globalization.Language", ILanguageFactory, LanguageFactory)
      CreateClass("Windows.Graphics.Imaging.BitmapDecoder", IBitmapDecoderStatics, BitmapDecoderStatics)
      CreateClass("Windows.Media.Ocr.OcrEngine", IOcrEngineStatics, OcrEngineStatics)
   ...
   if (file = "ShowAvailableLanguages") {
      if (GlobalizationPreferencesStatics = "")
         CreateClass("Windows.System.UserProfile.GlobalizationPreferences", IGlobalizationPreferencesStatics, GlobalizationPreferencesStatics)
   ...
   BitmapFrame := ComObjQuery(BitmapDecoder, IBitmapFrame)
   ...
   Close := ComObjQuery(IRandomAccessStream, IClosable)
   DllCall(NumGet(NumGet(Close+0)+6*A_PtrSize), "ptr", Close)   ; Close
   ObjRelease(Close)
   Close := ComObjQuery(SoftwareBitmap, IClosable)
   DllCall(NumGet(NumGet(Close+0)+6*A_PtrSize), "ptr", Close)   ; Close
   ObjRelease(Close)
...
WaitForAsync(ByRef Object)
{
   ...
   AsyncInfo := ComObjQuery(Object, IAsyncInfo)
   loop
   {
