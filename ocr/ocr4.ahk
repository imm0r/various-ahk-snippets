...
ocr(file, lang := "FirstFromAvailableLanguages")
{
   ...
   if (file = "ShowAvailableLanguages")
   {
      ...
      loop % count
      {
         DllCall(NumGet(NumGet(LanguageList+0)+6*A_PtrSize), "ptr", LanguageList, "int", A_Index-1, "ptr*", hString) ;
   ...
   if (lang != CurrentLanguage) or (lang = "FirstFromAvailableLanguages")
   {
      ...
      ​​else
      {
         ...
         DllCall(NumGet(NumGet(OcrEngineStatics+0)+9*A_PtrSize), "ptr", OcrEngineStatics, ptr, Language, "ptr*", OcrEngine) ;


      {
         msgbox Can't use language "%lang%" for OCR, please install language pack.
         ExitApp
      }
   ...
   DllCall("ShCore\CreateRandomAccessStreamOnFile", "wstr", file, "uint", Read :
   DllCall(NumGet(NumGet(BitmapDecoderStatics+0)+14*A_PtrSize), "ptr", BitmapDecoderStatics, "ptr", IRandomAccessStream, "ptr*", BitmapDecoder) ; CreateAsync
   WaitForAsync(BitmapDecoder)
   BitmapFrame := ComObjQuery(BitmapDecoder, IBitmapFrame := "{72A49A1C-8081-438D-91BC-94ECFC8185C6}")
   DllCall(NumGet(NumGet(BitmapFrame+0)+12*A_PtrSize), "ptr", BitmapFrame , "uint*", width) ; get_PixelWidth
   DllCall(NumGet(NumGet(BitmapFrame+0)+13*A_PtrSize), "ptr", BitmapFrame, "uint*", height) ; get_PixelHeight
   if (width > MaxDimension) or (height >
   ...
   DllCall(NumGet(NumGet(OcrEngine+0)+6*A_PtrSize), "ptr", OcrEngine, ptr, SoftwareBitmap, "ptr*", OcrResult) ;
