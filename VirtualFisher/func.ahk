...
GetLastMsg(text)
{
   if(text)
   {
      ...
      loop, % tLoop
      {
         tMsgHandler := tMsgHandler "`n" oText[LastMsgStart]
         LastMsgStart++
         if instr(oText[LastMsgStart], "Fish Again") || instr(oText[LastMsgStart], "NAchricht an #") instr(oText[LastMsgStart], "Sell") ||  instr(oText[LastMsgStart], "Return")
            break
      }
      return tMsgHandler
   }
