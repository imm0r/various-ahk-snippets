...
createGui() {
	...
	Gui, Cmd:Add, Text, x5 y5 readonly w350 h50 center vLvl_Descript, Enter the weekly donation points for clan member:
	Gui, Cmd:Font, s16, % "Bahnschrift SemiCondensed"
    Gui, Cmd:Add, Text, x5 y55 w165 h35 vLbl_CurrClanMember readonly right, % aList_ClanMembersMain[cm_Curr]
    Gui, Cmd:Add, Edit, x185 y54 w60 h30 vEdit_CurrDonation center number, 0
	Gui, Cmd:Font, s12, % "Bahnschrift SemiCondensed"
	Gui, Cmd:Add, Button, g_SaveAndExit vLbl_SaveAndExit x135 y115 h30 hidden1, Save and Exit
	Gui, Cmd:Add, Button, g_AddToTotal vLbl_AddToTotal x135 y115 h30 hidden1, Transfer data now
	Gui, Cmd:Add, Button, g_AddDonation vLbl_AddDonation x105 y115 h30, &Add donations to member
...
_AddDonation()
{
    ...
    if (cm_Amount = cm_Curr)
    {
        ...
        GuiControl, Show, Lbl_AddToTotal
        cm_Curr := 1
    }
    else
    {
        cm_Curr++
        GuiControl,, Lbl_CurrClanMember, % aList_ClanMembersMain[cm_Curr]
...
_AddToTotal()
{
    ...
    GuiControl, Hide, Lbl_AddToTotal
    GuiControl, Show, Lbl_SaveAndExit
    GuiControl, Text, Lvl_Descript, All data successfully added, you may close this window now.`nExcel file will be saved and exits also.
}
