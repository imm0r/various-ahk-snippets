...
Workflow:
  1. Download schema.min.json from GitHub (cache with 24h age check)
  2. Calculate Stats.datc64 column offsets from PoE2 schema (validFor=2)
  3. Decompress _.index.bin to find Stats.datc64 bundle entry
  4. Decompress the relevant bundle
  5. Parse Stats.datc64 binary (Id string + HASH32 columns)
  6. Write stat_name_map.tsv (HASH32_signed_i32 <tab> stat_name)
...
def parse_dat64_stats(dat_bytes: bytes, row_size: int, id_col_offset: int, hash32_col_offset: int = 0):
    """
    Parse a Stats.datc64 binary blob.
    Returns list of (row_idx: int, stat_id: str) tuples.
    Row index (0-based) matches the key stored in PoE2 character stat pairs.
    """
    ...
    # String offsets in .datc64 are relative to var_section_start (the 0xBB*8 magic is at offset 0)
    # Do NOT skip the 8 magic bytes when resolving offsets.
    var_data_base = var_section_start
    ...
    for row_idx in range(num_rows):
        ...
        # .datc64 strings: first 4 bytes of 8-byte column = uint32 offset into variable section
        str_offset = struct.unpack_from('<I', dat_bytes, row_base + id_col_offset)[0]

        # Resolve string from variable section
        # Offset 0 = start of the 0xBB*8 magic block; actual string data starts at offset 8
        str_pos = var_data_base + str_offset
        if str_pos >= len(dat_bytes):
            continue
        
        # .datc64 strings are UTF-16LE, null-terminated (look for 2 zero bytes)
        end = str_pos
        ...
        if stat_id:
            results.append((row_idx, stat_id))
...
data_dir    = os.path.join(os.path.dirname(script_dir), 'data')
    ...
    # Show sample
    for row_idx, name in entries[:5]:
        print(f'  row={row_idx:5d}  id={name}')
    ...
    with open(output_tsv, 'w', encoding='utf-8', newline='\n') as f:
        ...
        f.write(f'# key=row_index (0-based), matches PoE2 character stat pair keys in memory\n')
