...
#include <ShinsImageScanClass>  ;remove ..\ if the class file is in the same directory
scan := new ShinsImageScanClass("Unbenannt - Paint")
...
msgbox % x "," y "`n" scan.height "`n" scan.width
...
while(x < scan.width) {
	...
	x++
	;msgbox % x "," y "`n" dectohex(scan.getpixel(x,y))
}
msgbox % x "," y "`n" scan.getpixel(x,y)
...
scan.ClickDrag(100,100,600,600) ;cross1
scan.ClickDrag(regionX1+80,regionY2-80,regionX2-80,regionY1+80) ;cross2
msgbox, % regionX1 " - " regionY1 " - " regionX2 " - " regionY2
return
exitapp
